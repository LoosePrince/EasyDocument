...
| 配置项 | 类型 | 默认值 | 说明 |
|-------|------|-------|------|
| `root_dir` | 字符串 | `"/data"` | 文档根目录（前端路径） |
| `branch_support` | 布尔值 | `true` | 是否启用分支支持（多版本/多目录） |
| `default_branch` | 字符串 | `"main"` | 默认分支名称 |
| `available_branches` | 数组 | `[{ name, label }, ...]` | 可用分支列表，与 data/ 下目录一致 |
...
| `code_block` | 对象 | 见下文 | 代码块行号与主题 |
| `external_docs` | 数组 | `[]` | 外部文档挂载配置（仅默认分支生效） |
...
#### branch_support、default_branch、available_branches

当文档按多分支（如 main、1.3.4）组织时使用：

- `branch_support`: 设为 `true` 时，侧栏可切换分支，路径基于 `root_dir` 下各子目录（与 `data/` 下目录一致）。
- `default_branch`: 默认选中的分支名。
- `available_branches`: 分支列表，每项 `{ name: "目录名", label: "显示名" }`。
...
用于挂载外部文档源（仅在 `default_branch` 下生效，不参与版本分支切换）。
...
document: {
  external_docs: [
    {
      mount_path: "外部文档",
      mode: "path_json", // path_json | github_tree
...
- `mount_path`: 挂载目录路径；若为空或根路径，会合并到站点根目录；若命中已有路径则执行合并而非覆盖
- `mode: "path_json"`: 读取外部 `path.json`，并根据其相对路径加载文档
- `mode: "github_tree"`: 通过 GitHub API 读取目录结构；若目录存在 `path.json` 则优先使用
- 外部来源节点在侧边栏会显示来源标识，可悬停查看来源并点击跳转
...
文档页的 Git 展示**优先**使用 `build.py` 写入的 `path.json.git`；仅在缺少该字段时再使用 GitHub API。
...
document: {
  ...
  index_pages: ["README.md", "README.html", "index.md", "index.html"],
